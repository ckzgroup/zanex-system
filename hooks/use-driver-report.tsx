...
const useDriverRequest = (companyId: any) => {
            ...
            try {
                ...
                const detailedActiveRequests = await Promise.all(
                    activeRequests.map(async (request: any) => {
                        const detailURL = `${process.env.NEXT_PUBLIC_API_URL}/driver_cost_more.php?user_id=${request.user_id}&title=all`;
                        ...
                        const transformedRequest = {
                            ...request,
                            ...requestDetails, // Assuming there's only one detail, adjust if needed
                            details: undefined,
                        };
