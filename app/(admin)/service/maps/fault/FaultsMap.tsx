...
// Import the fixed Mapbox package entry point directly
...
const FaultsMap: React.FC = () => {
  ...
  // Track standard viewport states for Mapbox control
  ...
  }, [maps, selectedService, selectedCustomerId, selectedTicketStatus, selectedSite, selectedSlaStatus, selectedType]);
  ...
  useEffect(() => {
    const mapPoints = filteredMaps.map((map: any) => ({
      ...
      id: map.ticket_no,
      service_title: map.service_name,
      ...
      ticket_action_description: map.fault_description,
      sla_status: map.ticket_state,
    ...
    if (JSON.stringify(mapPoints) !== JSON.stringify(markers)) {
      ...
      const newCenter = mapPoints.length > 0 ? calculateCenter(mapPoints) : { lat: DEFAULT_LAT, lng: DEFAULT_LNG };
      setViewport(prev => ({
        ...prev,
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 gap-6 md:grid-cols-3">
          ...
          <div className="space-y-2">
            ...
            <Select onValueChange={(value) => setSelectedSite(value)} disabled={!selectedCustomerId}>
              ...
              <SelectContent>
                {sitesLoading ? (
                  <p className="p-2 text-sm">Loading sites...</p>
                ) : (
                  sites.length > 0 ? (
                    sites.map((site) => (
                    ...
                    <p className="p-2 text-sm">No sites available</p>
                  )
                )}
              </SelectContent>
            </Select>
            ...
            <Select onValueChange={(value) => setSelectedType(value)}>
              ...
              <SelectContent>
                {filteredMaps.length > 0 ? (
                  // Filter uniquely by fault type string values
                  Array.from(new Set(filteredMaps.map((i: any) => i.fault_type))).map((type: any, index) => (
                    <SelectItem key={index} value={`${type}`}>
                      {type || "Unknown Type"}
                    </SelectItem>
                  ))
                ) : (
                  <p className="p-2 text-sm">No Fault Types</p>
                )}
              </SelectContent>
            </Select>
          </div>
      ...
      {/* Map Box Component with Native Custom Markers */}
