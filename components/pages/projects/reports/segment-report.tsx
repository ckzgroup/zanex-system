...
function SegmentReport() {
  ...
  const {
    isLoading: segLoadiing,
    error: segError,
    data: segData,
  } = useSingleSegment("/project_segment/getSegment", segment_id);
  const segment = Array.isArray(segData) ? segData.reverse() : [];
  const { segment_name } = segment[0] || {};
  ...
  async function onSubmit(data: FormData) {
    try {
      ...
      const url = `https://k-task.ckzgroup.com/upload/fpdf/generate/segment_report.php?${queryParams}`;
