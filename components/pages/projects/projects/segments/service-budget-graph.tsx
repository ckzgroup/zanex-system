...
const ServiceBudgetGraph = ({ segment_id }: { segment_id: number }) => {
    const { isLoading, error, data } = useSingleSegment("/budget/getSegmentExpenditure", segment_id);

    const chartData = useMemo(() => {
        return data
            ?.filter((item: any) => item.budget_item_type === "Labour") // Filter only "Labour" items
            .map((item: any) => ({
...
export default ServiceBudgetGraph;
